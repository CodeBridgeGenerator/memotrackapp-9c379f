...
import notifyFakerFactory from "./notifyFakerFactory";
...
const NotifyFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = notifyFakerFactory(props.user,fakerCount,TajukIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("notify").create(elem));
...
export default connect(mapState, mapDispatch)(NotifyFakerDialogComponent);
