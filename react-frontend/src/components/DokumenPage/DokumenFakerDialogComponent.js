...
import dokumenFakerFactory from "./dokumenFakerFactory";
...
const DokumenFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [TajukIds, setTajukIds] = useState();
    ...
    useEffect(() => {
        ...
        client.service("latihan")
                        ...
                        .then((data) => {
                        setTajukIds(data.data);
                        });

    }, [props.show]);

    const onRun = async () => {
        let fakeData = dokumenFakerFactory(props.user,fakerCount,TajukIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("dokumen").create(elem));
...
export default connect(mapState, mapDispatch)(DokumenFakerDialogComponent);
