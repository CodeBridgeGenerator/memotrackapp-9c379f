...
import DokumenDatatable from "./DokumenDataTable";
...
import DokumenFakerDialogComponent from "./DokumenFakerDialogComponent";
import DokumenSeederDialogComponent from "./DokumenSeederDialogComponent";

const DokumenPage = (props) => {
    ...
    const filename = "dokumen.csv";

    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("dokumen");
    ...
    useEffect(() => {
        ...
        client
            .service("dokumen")
            .find({ query: { $limit: 10000 , Tajuk : urlParams.singleLatihanId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("dokumen").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Dokumen", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
        ...
        const promises = data.map((e) => client.service("dokumen").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
        ...
        <div className="mt-5">
            <div className="grid">
                <div className="col-6 flex justify-content-start">
                    <h3 className="mb-0 ml-2">Dokumen </h3>
                </div>
                <div className="col-6 flex justify-content-end">
                    <>
                    <Button label="add" loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="dokumen-add-button"/>
            ...
            <div className="grid align-items-center">
                <div className="col-12" role="dokumen-datatable">
                    <DokumenDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>
            ...
            <DokumenEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <DokumenCreateDialogComponent entity={newRecord} show={showCreateDialog} onHide={() => setShowCreateDialog(false)} onCreateResult={onCreateResult} />
            <DokumenFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <DokumenSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(DokumenPage);
