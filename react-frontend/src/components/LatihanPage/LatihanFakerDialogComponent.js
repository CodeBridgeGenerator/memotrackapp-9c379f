...
const LatihanFakerDialogComponent = (props) => {
    ...
    const [KategoriIds, setKategoriIds] = useState();
    ...
    useEffect(() => {
        ...
        client.service("category")
                        .find({ query: { $select: ["_id","Category"] } })
                        .then((data) => {
                        setKategoriIds(data.data);
                        });

    }, [props.show]);

    const onRun = async () => {
        let fakeData = latihanFakerFactory(props.user,fakerCount,KategoriIds);
