...
import kelulusanFakerFactory from "./kelulusanFakerFactory";
...
const KelulusanFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [LatihanIds, setLatihanIds] = useState();


    useEffect(() => {
        setFakerCount(1);
       
        client.service("latihan")
                        .find({ query: { $select: ["_id","Tajuk"] } })
                        .then((data) => {
                        setLatihanIds(data.data);
                        });

    }, [props.show]);

    const onRun = async () => {
        let fakeData = kelulusanFakerFactory(props.user,fakerCount,LatihanIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("kelulusan").create(elem));
...
export default connect(mapState, mapDispatch)(KelulusanFakerDialogComponent);
