...
import calonFakerFactory from "./calonFakerFactory";
...
const CalonFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = calonFakerFactory(props.user,fakerCount,LatihanIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("calon").create(elem));
...
export default connect(mapState, mapDispatch)(CalonFakerDialogComponent);
