...
const CalonCreateDialogComponent = (props) => {
    ...
    const validate = () => {
        ...
        const error = {};
            ...
            if (_.isEmpty(_entity?.Pengguna)) {
                error["Pengguna"] = `Pengguna field is required`;
                ret = false;
            }
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            Pengguna: _entity?.Pengguna,Latihan: _entity?.Latihan?._id,Status: _entity?.Status,
    ...
    return (
        <Dialog header="Create Calon" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            ...
            <span className="align-items-center">
                ...
                <InputText id="Pengguna" className="w-full mb-3 p-inputtext-sm" value={_entity?.Pengguna} onChange={(e) => setValByKey("Pengguna", e.target.value)}  required  />
            </span>
